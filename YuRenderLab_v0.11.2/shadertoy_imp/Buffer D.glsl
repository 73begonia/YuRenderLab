...
//   iChannel0 = Buffer D  (self, filter: nearest, wrap: clamp)
//   iChannel1 = Keyboard  (filter: nearest, wrap: clamp)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) 
{
    ...
    float spd = CAM_SPEED;
    if (keyDown(iChannel1, KEY_SHIFT)) spd *= CAM_SPRINT;

    if (keyDown(iChannel1, KEY_W))     camPos += fwd * spd;
    if (keyDown(iChannel1, KEY_S))     camPos -= fwd * spd;
    if (keyDown(iChannel1, KEY_A))     camPos -= rt  * spd;
    if (keyDown(iChannel1, KEY_D))     camPos += rt  * spd;
    if (keyDown(iChannel1, KEY_SPACE)) camPos.y += spd;
    if (keyDown(iChannel1, KEY_CTRL))  camPos.y -= spd;
