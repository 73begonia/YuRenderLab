...
// Diffuse IBL via spherical harmonics (Buffer A).
// Specular IBL via split-sum prefiltered env map + BRDF LUT (Buffer B).
//
// Shadertoy channel setup:
//   iChannel0 = Buffer D  (camera state, filter: nearest, wrap: clamp)
//   iChannel1 = Buffer A  (SH + env map, filter: linear, wrap: repeat)
//   iChannel2 = Buffer B  (prefiltered specular + BRDF, filter: linear, wrap: clamp)
...
float getRoughness(int matId) {
    if (matId == MAT_SPHERE) return 0.05;
    if (matId == MAT_TALL)   return 0.6;
    return 0.8;
}
...
// Get environment colour from equirectangular projection in Buffer A
...
// Get interpolated prefiltered environment for roughness from Buffer B atlas
